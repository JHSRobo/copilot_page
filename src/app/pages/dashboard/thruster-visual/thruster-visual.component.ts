...
export class ThrusterVisualComponent implements OnInit{
  ...
  ngOnInit() {
    this.rovService.topic('horizontalDrive').data.subscribe(v => {
      this.thrusters[0] = this.convertToPercent(v.t1);
      this.thrusters[1] = this.convertToPercent(v.t2);
      this.thrusters[4] = this.convertToPercent(v.t3);
      this.thrusters[5] = this.convertToPercent(v.t4);
    });

    this.rovService.topic('verticalDrive').data.subscribe(v => {
      this.thrusters[2] = this.convertToPercent(v.t1);
      this.thrusters[3] = this.convertToPercent(v.t2);
    });
  }

  convertToPercent(thrusterValue) {
    return Math.round((((thrusterValue / 10) + 100) / 2));
  }
}
